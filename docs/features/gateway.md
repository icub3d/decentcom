...
## Requirements
- [ ] Clients can open a WebSocket connection to `GET /api/v1/gateway` with a session token.
- [ ] The server validates the session token on connect and rejects invalid or expired tokens with an appropriate close code.
- [ ] The server sends a `HELLO` event upon successful connection containing the server name, user info, and list of channels.
- [ ] Clients can subscribe to specific channels by sending a `SUBSCRIBE` command with a channel ID.
- [ ] Clients can unsubscribe from channels by sending an `UNSUBSCRIBE` command.
- [ ] The server broadcasts events only to clients subscribed to the relevant channel (plus global events sent to all connected clients).
- [ ] The server sends a `HEARTBEAT` ping at a configurable interval (default 30 seconds).
- [ ] Clients must respond with a `HEARTBEAT_ACK` within a timeout (default 10 seconds) or the server closes the connection.
- [ ] On disconnect, the server cleans up all subscriptions and presence state for that session.
- [ ] The server supports multiple concurrent connections per user (one per device).
...
- [ ] Define `Op` enum and `EventEnvelope<T>` in the shared types crate (`shared/src/gateway.rs`).
- [ ] Define `ClientCommand` enum (SUBSCRIBE, UNSUBSCRIBE, HEARTBEAT_ACK) in shared types.
- [ ] Implement `ConnectionRegistry` in `server/src/gateway/registry.rs` with methods: `register`, `unregister`, `subscribe`, `unsubscribe`, `broadcast_to_channel`, `send_to_user`, `broadcast_all`.

### Phase B: Connection handler
- [ ] Implement the axum WebSocket upgrade handler in `server/src/gateway/handler.rs`. Extract and validate the session token. Reject unauthorized connections with WS close code 4001.
- [ ] Implement the per-connection actor using tokio::select! for read/write/heartbeat.
- [ ] Implement heartbeat logic: server sends HEARTBEAT on interval, closes connection if HEARTBEAT_ACK is not received within timeout.

### Phase C: Subscribe/unsubscribe and event dispatch
- [ ] Handle SUBSCRIBE commands: add session to channel subscription set.
- [ ] Handle UNSUBSCRIBE commands: remove session from subscription set.
- [ ] Send HELLO event on successful connection with server info, user data, and channel list.
- [ ] Wire the gateway route into the axum router in `server/src/main.rs`.

### Phase D: Broadcast API for other features
- [ ] `GatewayHandle` type alias (= `ConnectionRegistry`) — cheaply cloneable, held in `AppState`. Other modules call `broadcast_to_channel`, `send_to_user`, `broadcast_all`.

## Test List
- [ ] Unit test: `ConnectionRegistry` correctly tracks registrations, subscriptions, and cleanup on unregister.
- [ ] Unit test: `EventEnvelope` serializes to the expected JSON format (via shared crate roundtrip).
- [ ] Unit test: `ClientCommand` deserializes from JSON correctly, including unknown ops.
- [ ] Integration test: Connect to gateway with valid session token, receive HELLO event.
- [ ] Integration test: Connect with invalid/expired token, connection is rejected.
- [ ] Integration test: Subscribe to a channel, receive events broadcast to that channel, unsubscribe and stop receiving them.
- [ ] Integration test: Heartbeat timeout -- connect, do not respond to HEARTBEAT, verify server closes connection.
- [ ] Integration test: Multiple clients connected simultaneously receive independent event streams.
- [ ] Manual: Open a WebSocket connection using `websocat` or similar tool, verify HELLO and heartbeat cycle.
